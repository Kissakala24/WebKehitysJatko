...
const resourceValidators = [
  ...
  body('resourceName')
    ...
    .trim()
    .isLength({ min: 1 }).withMessage('resourceName must not be empty')
    .not().matches(/<|>/).withMessage('resourceName must not contain HTML characters')
    .escape(),
  ...
  body('resourceDescription')
    ...
    .not().matches(/<[^>]*>/).withMessage('resourceDescription must not contain HTML tags'),

  body('resourceAvailable')
    .exists({ checkNull: true }).withMessage('resourceAvailable is required')
